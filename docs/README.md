...
| [`SPEC-02-relay-and-edge.md`](specs/SPEC-02-relay-and-edge.md) | 🟢 **Built + hardware-validated.** Modes 1+2 → dashboard. Byte accounting, the shared flag helper, SSE, `/latest.jpg`, ring buffer. **Includes the measured results** — the real ratio is **~2,395×, not 689×**. |
| [`SPEC-03-web-dashboard.md`](specs/SPEC-03-web-dashboard.md) | 🟢 **Built + browser-verified.** The browser: split status/lesson layout, the video panel (whose *emptiness* in Mode 2 is the privacy lesson), 60 s chart, event log, offline-only assets. Since extended with the SPEC-06 sliders, the SPEC-07 mode buttons and the SPEC-09 alarm. |
...
| [`SPEC-08-mode3-multimodal-and-preview.md`](specs/SPEC-08-mode3-multimodal-and-preview.md) | 🟢 **Built + hardware-validated 2026-07-16.** Two deliberate reversals of SPEC-04, both Jeffry's call: **Part A** — Mode 3 fuses *sound* as corroboration (a thump fires the fall in 1 s instead of 3; a dead mic degrades to exactly the old vision-only rule, never gates). **Part B** — the "show camera (setup)" toggle, the ONE way pixels may leave Mode 3: default off, non-sticky, its own byte bucket. |
| [`SPEC-09-fall-alarm-sound.md`](specs/SPEC-09-fall-alarm-sound.md) | 🟢 **Built + browser-verified 2026-07-17.** The dashboard *sounds* a fall: synthesized two-tone siren + spoken "Fall detected" (中文 dashboards speak 中文), repeating while FALL? is active, any mode, ngrok viewers included. No file shipped, no mute toggle — the first click unlocks audio. Latches two full cycles (~8 s, sized by ear) because `fall_suspected` is a 1-second pulse (caught on hardware). By-ear check on the venue machine still owed. |
...
| [`13-ngrok-demo-tunnel.md`](03-tooling/13-ngrok-demo-tunnel.md) | **Showing the live dashboard to someone who is not in the room.** Demo transport only — it changes nothing about the edge/cloud split or the byte accounting. Verified on the free tier: the tunnel works and **SSE survives it** (the real risk — proxies love to buffer streaming responses into uselessness). **🛑 The dashboard has NO auth of its own**, so a bare `ngrok http 8000` hands anyone with the link the room's camera *and* the ability to switch modes — always pass `--basic-auth` (works free; verified 401/200). **⚠️ Never put credentials in the URL**: Chrome then refuses to build any `fetch()` from the page, so `/config` never loads and the sliders read `—` — which looks exactly like a broken dashboard and is not. |
